      ...
      <div className="min-h-screen bg-black pt-32 pb-16">
        <div className="container mx-auto px-4">
          ...
          >
            ...
            <Link
              href="/catalogo"
              className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-flow-pink to-flow-cyan text-white rounded-lg font-medium hover:opacity-90 transition-opacity"
            >
              Ver catálogo
            </Link>
          </motion.div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {items.map((item, index) => (
              <motion.div
                ...
                className="bg-dark-gray rounded-xl p-4 md:p-6 border border-white/10"
              >
                <div className="flex gap-4 md:gap-6">
                  {/* Image */}
                  <Link href={`/catalogo/${item.product.slug}`} className="flex-shrink-0">
                    <div className="w-20 h-20 md:w-24 md:h-24 rounded-lg overflow-hidden bg-charcoal">
                      <img
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between gap-4">
                      <div>
                        ...
                        <Link href={`/catalogo/${item.product.slug}`}>
                          <h3 className="text-white font-bold hover:text-flow-cyan transition-colors line-clamp-1">
                            {item.product.name}
                          </h3>
                        </Link>
                        <p className="text-white/60 text-sm">{item.product.league}</p>
                      </div>
                      <button
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      />
                      <span className="text-white/50 text-xs">Color seleccionado</span>
                    </div>
            ...
            {getTotalItems() < 6 && (
              <div className="bg-flow-yellow/10 border border-flow-yellow/30 rounded-xl p-4">
                <p className="text-flow-yellow font-medium text-center">
          ...
          <div className="lg:col-span-1">
            <motion.div
              ...
              className="bg-dark-gray rounded-xl p-6 border border-white/10 sticky top-32"
            >
              <h2 className="text-xl font-bold text-white mb-6">Resumen del pedido</h2>

              {/* Delivery options */}
              <div className="mb-6">
                <p className="text-white/60 text-sm mb-3">Método de entrega:</p>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-3 p-3 rounded-lg border transition-all ${
                        deliveryMethod === option.id
                          ? 'border-flow-cyan bg-flow-cyan/10'
              ...
              <div className="mt-6 pt-6 border-t border-white/10">
                <p className="text-white/60 text-sm mb-3">Descuentos por volumen:</p>
                <div className="space-y-2 text-sm">
                  <div className={`flex justify-between ${getTotalItems() >= 6 && getTotalItems() < 12 ? 'text-green-400' : 'text-white/50'}`}>
                    <span>6-11 unidades</span>
                    <span>10% descuento</span>
                  </div>
                  <div className={`flex justify-between ${getTotalItems() >= 12 && getTotalItems() < 24 ? 'text-green-400' : 'text-white/50'}`}>
                    <span>12-23 unidades</span>
                    <span>15% descuento</span>
                  </div>
                  <div className={`flex justify-between ${getTotalItems() >= 24 ? 'text-green-400' : 'text-white/50'}`}>
